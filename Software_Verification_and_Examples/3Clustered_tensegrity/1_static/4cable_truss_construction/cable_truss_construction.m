...
%% 
index_prestressing=1;%1 for adding prestress in bottom string;0 for lifting construction
%% add extra boundary nodes
Nxy=diag([1,1,0])*N(:,1:80);
N2=[N,N(:,1:80)+0.001*Nxy/diag(sqrt(sum(Nxy.^2,1)))];            %add more nodes as boundary, decrease Z coordinate

%% add lifting strings
% connectivity matrix 
C_s_in_lift = [[1:80]', 440+[1:80]'];  % Write down the connection of bars
C_s_lift = tenseg_ind2C(C_s_in_lift ,N2); %get the matrix of relation through C_b_in
C_b2=[C_b,zeros(nb,80)];
C_s2=[[C_s,zeros(numel(index_s),80)];C_s_lift];

C2 = [C_b2;C_s2];
[ne2,nn2]=size(C2);        % ne:No.of element;nn:No.of node
index_s2=setdiff([1:ne2]',index_b);

% area
A2=[A;5e-4*ones(80,1)];     % add area of lifting strings
E_c2=E_c(1)*ones(ne2,1);
% rest length
H2=N2*C2';                     % element's direction matrix
l2=sqrt(diag(H2'*H2));         % elements' length
t2=[t;zeros(80,1)];
l02=E_c2.*A2.*l2./(t2+E_c2.*A2);

mass2=rho(1).*A2.*l02;


%% mass matrix and damping matrix
M=tenseg_mass_matrix(mass2,C2,lumped); % generate mass matrix
% damping matrix
d=0;     %damping coefficient
D2=d*2*max(sqrt(mass2.*E_c2.*A2./l02))*eye(3*nn2);    %critical damping


%% renew group matrix
gr={};  % outer diagonal, inner diagonal, inner hoop
Gp2=tenseg_str_gp3(gr,C2);    %generate group matrix
% S=eye(ne);                  % no clustering matrix
S2=Gp2';                      % clustering matrix is group matrix
tenseg_plot_CTS(N2,C2,[],S2);
%% Change Boundary constraints
if index_prestressing==1
    pinned_X=([1:40,nn+1:nn2])'; pinned_Y=([1:40,nn+1:nn2])'; pinned_Z=([1:40,nn+1:nn2])';
else
pinned_X=(nn+1:nn2)'; pinned_Y=(nn+1:nn2)'; pinned_Z=(nn+1:nn2)';
end
[Ia2,Ib2,a2,b2]=tenseg_boundary(pinned_X,pinned_Y,pinned_Z,nn2);
...
ind_dnb=[]; dnb0=[];
if index_prestressing==1
ind_dl0_c=[ne+41:ne2];dl0_c=[0.5*ones(40,1)];              % extend bottom cables 
else
    ind_dl0_c=[ne+1:ne2];  dl0_c=[10*ones(80,1)];              % extend both cables 
end
[w_t,dnb_t,l0_ct,Ia_new,Ib_new]=tenseg_load_prestress(substep,ind_w,w,ind_dnb,dnb0,ind_dl0_c,dl0_c,l02,b2,gravity,[0;0;9.8],C2,mass2);
% rest length
...
% input data
data.N=N2; data.C=C2; data.ne=ne2; data.nn=nn2; data.Ia=Ia_new; data.Ib=Ib_new;data.S=S2;
data.E=E_c2; data.A=A2; data.index_b=index_b; data.index_s=index_s2;
...
changzhou=1;    %1 changzhou, 0 duanzhou 
if changzhou                
